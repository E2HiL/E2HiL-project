...
def add_actor_information_and_train(
    ...
    replay_buffer = initialize_replay_buffer(cfg, device, storage_device) # device = cuda
    batch_size = cfg.batch_size
    offline_replay_buffer = None

    if cfg.dataset is not None:
        offline_replay_buffer = initialize_offline_replay_buffer( # device = cuda
            cfg=cfg,
            device=device,
    ...
    while True:
        ...
        # Count tags to separate offline vs online samples
        if "tag" in batch:
            tag_values = batch["tag"].cpu().numpy()
            tag_1_count = (tag_values == 1).sum()
            tag_0_count = (tag_values == 0).sum()
            print(f"Count of offline buffer: {tag_1_count}, Count of online buffer: {tag_0_count}")
        ...
        if optimization_step % policy_update_freq == 0:
            for _ in range(policy_update_freq):
                ...
                # Metrics produced by compute_loss_actor_with_cov (some envs may not return them)
                training_infos.update({
                    "policy_entropy_in_actor":               actor_output.get("policy_entropy_in_actor"),
                    "policy_entropy_delta":         actor_output.get("policy_entropy_delta"),
                    "entropy_change_pred":          actor_output.get("entropy_change_pred"),
                    "cov_logp_softadv":             actor_output.get("cov_logp_softadv"),
                })
                cov_per_sample = actor_output.get("cov_per_sample")
                
                # Persist covariance per sample alongside its tag to JSON
                if cov_per_sample is not None and "tag" in batch:
                    tag_values = batch["tag"].cpu().numpy()
                    cov_data = {
                    ...
                    for i, (tag, cov) in enumerate(
                        zip(tag_values, cov_per_sample)
                    ):
                        cov_data["samples"].append({
                            "sample_idx": int(i),
                            "tag": int(tag),
                            "cov_per_sample": float(cov)
                        })
                    
                    # Save to JSON file
                    cov_log_dir = os.path.join(cfg.output_dir, "cov_logs")
                    os.makedirs(cov_log_dir, exist_ok=True)
                    cov_log_file = os.path.join(
                        cov_log_dir, f"cov_step_{optimization_step}.json"
                    )
                    with open(cov_log_file, "w") as f:
                        json.dump(cov_data, f, indent=2)
                        print(f"!Saved cov_per_sample log to {cov_log_file}")
                ...
                training_infos["policy_entropy"] = temperature_output["policy_entropy"]
                training_infos["target_entropy"] = temperature_output.get("target_entropy")
...
def push_actor_policy_to_queue(parameters_queue: Queue, policy: nn.Module):
    ...
    parameters_queue.put(state_bytes)  # Move to CPU before cross-process transfer
...
def process_interaction_message(
    ...
    # Rewrite episodic reward under a "robot completed independently" convention
    raw_reward = float(message.get("Episodic reward", 0) or 0.0)
    human_takeover_ratio = float(message.get("Intervention rate", 0.0) or 0.0)
    # If human takeover exceeds 20%, treat as failure; otherwise require env success (often raw_reward==1) to count as success
    robot_only_reward = 1 if (raw_reward >= 1.0 and human_takeover_ratio <= 0.20) else 0

    # Keep original values for analysis while exposing standard keys for W&B logging
    message["Episodic reward (raw)"] = raw_reward
    message["Episodic reward"] = robot_only_reward
    message["Human takeover ratio"] = human_takeover_ratio
    message["Intervention threshold"] = 0.70  # For visualization only
...
def process_transitions(
    ...
    while not transition_queue.empty() and not shutdown_event.is_set():
        ...
        for transition in transition_list:
            ...
            if is_intervention:
                transition["tag"] = torch.tensor(1, dtype=torch.long)
            else:
                transition.setdefault("tag", torch.tensor(0, dtype=torch.long))   # Default to 0
            replay_buffer.add(**transition)
            # Add to offline buffer if it's an intervention
            if dataset_repo_id is not None and transition.get("complementary_info", {}).get("is_intervention"):
